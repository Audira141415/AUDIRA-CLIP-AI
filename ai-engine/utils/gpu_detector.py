...
if __name__ == '__main__':
    ...
    args = detector.get_encoder_args()
    # Combine them into key-value pairs for fluent-ffmpeg outputOptions
    # fluent-ffmpeg prefers an array of strings like '-c:v h264_nvenc', '-preset p4'
    fluent_args = []
    i = 0
    while i < len(args):
        if args[i].startswith('-'):
            if i+1 < len(args) and not args[i+1].startswith('-'):
                fluent_args.append(f'{args[i]} {args[i+1]}')
                i += 2
            else:
                fluent_args.append(args[i])
                i += 1
        else:
            fluent_args.append(args[i])
            i += 1
    print(json.dumps(fluent_args))
